...
class SST(object):
    def __init__(self, robot_env: QuadcopterEnvV2):
        ...
        si.setPropagationStepSize(self.robot_env.dt)
    ...
    def obRealVector2array(self, realvector, length):
        n = length
        array = np.zeros(n)
        for i in range(n):
    ...
    def array2obRealVector(self, array, realvector, length):
        n = length
        for i in range(n):
    ...
    def set_start_and_goal(self, start: np.ndarray, goal: np.ndarray):
        ...
        self.array2obRealVector(start, self.start, len(self.state_bounds))
        self.array2obRealVector(goal, self.goal, len(self.state_bounds))

    def check_reach(self, path):
        goal_array = self.obRealVector2array(self.goal, len(self.state_bounds))
    ...
    def planning(self, runtime=50.0):
        ...
        for _ in np.arange(0.0, runtime+dt, dt):
            ...
            path_array = np.array([j.split()
                    for j in path_matrix.splitlines()], dtype=float)
            if self.check_reach(path_array):
